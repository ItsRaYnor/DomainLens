...
class UnexpectedIssuanceTests(unittest.TestCase):
    ...
    def test_an_unrecognised_issuer_is_still_surfaced(self):
        """Dropping it silently would let a genuinely unexpected certificate
        pass as "nothing to report"."""
        result = ct.analyse([_cert("CN=Some Obscure CA Ltd")], self.CAA, now=NOW)
        titles = [f["title"] for f in ct.findings(result, _fake_finding)]
        self.assertTrue(any("could not be checked" in t for t in titles), titles)
...
class MxDaneTests(unittest.TestCase):
    ...
    def test_no_mx_is_not_applicable_rather_than_a_failure(self):
        """A domain that receives no mail has no mail hosts to secure."""
        with mock.patch.object(s, "_resolve", return_value=[]):
            result = s._check_mx_dane("example.com")
        self.assertEqual("not_applicable", result["state"])
        self.assertIn("no mail hosts", result["reason"])
